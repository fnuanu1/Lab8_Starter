...
window.addEventListener('load', async event=>{
  ...
  //            log that it has failed.
    ...
    console.log("The service worker has NOT been successfully registered");
  }
...
async function getRecipes() {
  ...
  const recipest = localStorage.getItem("recipes");
  if ( recipest !== null){ //recipest !== null &&
    return JSON.parse(recipest);
  }
...
for(let i=0; i< RECIPE_URLS.length; i++){
  ...
  let rep = await fetch(RECIPE_URLS[i]);
 ...
 let recipe = await rep.json();
  ...
  if(i===RECIPE_URLS.length-1){
    saveRecipesToStorage(recipefetch);
    resolve(recipefetch);
  }
  } //try end
  catch(error){
